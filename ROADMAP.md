...
- [x] Settings is a full page with tabs (AI, Data), not a modal: key, narrator model autocomplete (default Claude Haiku 4.5), narrator instructions, storage state. A second model is not configured: the bets doc replaced the executor handoff with deterministic writes, and any worldbuilder model comes back only when that bet is spiked
