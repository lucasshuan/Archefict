...
- [x] Collapsible left sidebar: campaign list with create, switch and two-step delete; Settings entry; mock guest user. It participates in layout rather than covering it and remembers whether the user hid it. Icons via lucide-solid; contrast bug fixed (an unlayered rule in app.css beat every utility); title renames inline. Campaign list is a local registry (`campaign/library.ts`) until it becomes the account's list in Slice 11
...
- [x] Campaign navigation: Story and Library in a right sidebar, with Campaign Settings separated at its foot. It is an icon rail, like the left sidebar, and hides to a corner button. Every destination stays mounted; only the active one is displayed
...
- [x] ProseMirror bound to Automerge; one sheet document persisted to IndexedDB **(bet)** — built Sep 09, 2026 as the Library tab: tree panel (folders as their own entity, create/rename/archive/restore, remove empty folders), sheet panel (title, ProseMirror body, block commands in the panel `⋯`), fields panel. `@automerge/prosemirror` pre-bundled with ProseMirror and deduped in Vite, as the spike found
- [x] Library panel rebuilt (Sep 14, 2026): a search box over three icon tabs — sheets (the tree), models (flat, by title) and assets (empty, and says so). Search answers inside the open tab, flat and ranked, each row saying which folders it came from (`library/search.ts`, tested). Every row's actions moved into one `⋯`: open, rename, duplicate, archive for a sheet; make, rename, remove for a folder; the same four as a sheet for a conversation. A folder no longer hands models to sheets made inside it from the UI — the schema field and `createSheet`'s use of it stay, so campaigns that set one keep it. Duplicate clones the document rather than copying its fields, so marks and block structure survive
- [x] Archived is a dialog, not a fold (Sep 14, 2026), shared by the Story tab and the Library: restore one, delete one, or delete all for good. Deleting drops the record and its document, and always asks first — archiving is its own undo, deleting is not
- [x] Drag a sheet in the tree (Sep 14, 2026): between two sheets to reorder, onto a folder row to put it in, onto the ground below the tree to take it back out. `moveSheet` names the position as a neighbour rather than an index, so an archived sheet holding its slot cannot shift the answer. `Alt` and an arrow does the same from the keyboard. Folders themselves do not move yet
- [x] Free-form fields panel (key: value) on every sheet (Sep 09, 2026; values are text diffs, so two devices merge)
- [x] `/` command menu (Sep 14, 2026), the way Notion, Fibery and LegendKeeper do it: at the start of a block or after whitespace, listing what the block can become plus the two chips, each line naming the syntax that does the same thing. The menu and the toolbar share one command list (`library/commands.ts`), so neither can drift about what a heading is. *Place a field* left the toolbar the same day; no divider, because `horizontal_rule` has no `automerge` mapping to be stored as
