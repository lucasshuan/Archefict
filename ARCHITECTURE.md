...
- Three foreground tones: `fg` for content, `fg-muted` for labels and chrome, `fg-subtle` for supporting text. Section headings, their icons and value captions use the accent; the rule beside a heading stays in the border tone. Explanations belong in an info badge tooltip beside the label, not printed under the control; only text describing the chosen value stays on the page.
