...
> September 06, 2026. Converged after the Claude stack review and the GPT counter-review.
...
3. **Progressive formalization.** Unstructured, then ad-hoc fields, then components and types, then plugin-enhanced. Schema is never mandatory.
...
Sheet { id, body: rich text (blocks + marks), fields: free-form map,
        refs: stable ids, components?: ComponentId[], meta + provenance }
Component = schema fragment: claimed fields, UI hints, AI serializer hints. Several per sheet.
...
| Editor | ProseMirror + automerge-prosemirror | Not Tiptap, Milkdown, BlockNote, Lexical |
